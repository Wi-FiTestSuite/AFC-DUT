...
class AFCD_UAU_TV2_1(AFCBaseScript):
    def __init__(self):
    ...
    def execute(self):
        ...
        power_valid = RfMeasurementValidation(afc_resp["sentResponse"] , report).validate_rf_measurement_by_chan()
        ...
        if report:
            if lpi_support:                
                if not RfMeasurementValidation({} , report).validate_lpi_transmit_power():
                    InstructionLib.append_measurements(
                        "AFC_POWER_CONFORM_1", False, "DUT conforms to the conditons in no available frequency ragnes and/or channels")
                    return
            else:
                InstructionLib.log_error(f'The DUT should not transmit in the band if the DUT supports only SP operation')
                InstructionLib.append_measurements(
        ...
        InstructionLib.append_measurements("AFC_POWER_CONFORM_1", True, "DUT conforms to the conditons in no available frequency ragnes and/or channels")
